...
class SGLangRollout(BaseRollout):
    ...
    def _count_turns(messages: list[Message]) -> int:
        """Treat each assistant message as one turn."""
        if not messages:
            return 0
    ...
    def _count_tool_calls(messages: list[Message]) -> int:
        """Number of tool messages equals number of tool calls issued."""
        if not messages:
            return 0
        return sum(1 for message in messages if message.role == "tool")
    
    @staticmethod
    def _check_first_action_is_think(messages: list) -> bool:
        """Check if the first action (tool call) is 'think' or 'thinking'."""
        if not messages:
            return False
        for message in messages:
            # Handle both object and dict formats
            role = message.role if hasattr(message, "role") else message.get("role")
            tool_calls = getattr(message, "tool_calls", None) if hasattr(message, "tool_calls") else message.get("tool_calls")
            
            if role == "assistant" and tool_calls:
                # Return True if first tool call is 'think' or 'thinking'
                first_tool = tool_calls[0]
                tool_name = first_tool.function.name if hasattr(first_tool, "function") else first_tool.get("function", {}).get("name")
                return tool_name in ("think", "thinking")
        return False
    
    @staticmethod
    def _compute_think_length(messages: list) -> int:
        """Compute the total character length of all 'think'/'thinking' tool call arguments."""
        import json
        if not messages:
            return 0
        total_length = 0
            ...
            if role == "assistant":
                ...
                if tool_calls:
                    for tool_call in tool_calls:
                        ...
                        # Check if this is a thinking tool (think or thinking)
                        if tool_name in ("think", "thinking") and arguments:
                            # If arguments is a JSON string, parse it
                            if isinstance(arguments, str):
                                try:
                                    args_dict = json.loads(arguments)
                                    if isinstance(args_dict, dict):
                                        # Sum the lengths of goal, status, and next_step
                                        goal = args_dict.get("goal", "")
                                        status = args_dict.get("status", "")
                                        next_step = args_dict.get("next_step", "")
                                        total_length += len(str(goal)) + len(str(status)) + len(str(next_step))
                                    else:
                                        total_length += len(arguments)
                                except (json.JSONDecodeError, TypeError):
                                    total_length += len(arguments)
                            elif isinstance(arguments, dict):
                                # If arguments is already a dict
                                goal = arguments.get("goal", "")
                                status = arguments.get("status", "")
                                next_step = arguments.get("next_step", "")
                                total_length += len(str(goal)) + len(str(status)) + len(str(next_step))
                            else:
                                total_length += len(str(arguments))
        return total_length
        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    for tool_call in parsed_tool_calls:
                        tool_name = tool_call.function.name
                        _req.tool_invocation_counter[tool_name] = _req.tool_invocation_counter.get(tool_name, 0) + 1
                        # Track thinking length
                        if tool_name in ("think", "thinking"):
                            if not hasattr(_req, "thinking_length_total"):
                                _req.thinking_length_total = 0
                            ...
                            if isinstance(args, str):
                                try:
                                    ...
                                    if isinstance(args_dict, dict):
                                        goal = str(args_dict.get("goal", ""))
                                        status = str(args_dict.get("status", ""))
                                        next_step = str(args_dict.get("next_step", ""))
                                        _req.thinking_length_total += len(goal) + len(status) + len(next_step)
                                    else:
                                        _req.thinking_length_total += len(args)
                                except (json.JSONDecodeError, TypeError):
                                    _req.thinking_length_total += len(args)
                                ...
                                _req.thinking_length_total += len(str(args))
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        tool_call_counts_by_name: dict[str, list[int]] = {name: [] for name in self._tool_map.keys()}
        first_action_think_list: list[bool] = []
        think_length_list: list[int] = []
        if self.config.calculate_log_probs:
            output_logprobs = []
        ...
        for req in sorted_output_req_list:
            ...
            request_tool_counts = getattr(req, "tool_invocation_counter", None) or {}
            if tool_call_counts_by_name:
                for tool_name in tool_call_counts_by_name:
                    tool_call_counts_by_name[tool_name].append(request_tool_counts.get(tool_name, 0))
            if request_tool_counts:
                num_processed = len(tool_call_counts)
                for tool_name, count in request_tool_counts.items():
                    if tool_name not in tool_call_counts_by_name:
                        tool_call_counts_by_name[tool_name] = [0] * num_processed
                        tool_call_counts_by_name[tool_name].append(count)
            total_tool_calls = sum(request_tool_counts.values())
            if total_tool_calls == 0 and not request_tool_counts:
                total_tool_calls = self._count_tool_calls(req.messages)
            num_turns_list.append(self._count_turns(req.messages))
            tool_call_counts.append(total_tool_calls)
            first_action_think_list.append(self._check_first_action_is_think(req.messages))
            # Use the tracked thinking length from request object
            thinking_length = getattr(req, "thinking_length_total", 0)
            think_length_list.append(thinking_length)
            if self.config.calculate_log_probs:
        ...
        if num_turns_list:
            non_tensor_batch["__num_turns__"] = np.array(num_turns_list, dtype=np.int32)
        if tool_call_counts:
            non_tensor_batch["tool_call_counts"] = np.array(tool_call_counts, dtype=np.int32)
        if tool_call_counts_by_name:
            for tool_name, counts in tool_call_counts_by_name.items():
                non_tensor_batch[f"tool_call_counts/{tool_name}"] = np.array(counts, dtype=np.int32)
        if first_action_think_list:
            non_tensor_batch["__first_action_think__"] = np.array(first_action_think_list, dtype=bool)
        if think_length_list:
            non_tensor_batch["__think_length__"] = np.array(think_length_list, dtype=np.int32)
