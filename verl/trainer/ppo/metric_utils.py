...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    total_tool_counts = None
    if "tool_call_counts" in batch.non_tensor_batch:
        tool_call_counts = torch.as_tensor(batch.non_tensor_batch["tool_call_counts"]).float()
        metrics["tool_call_counts/mean"] = torch.mean(tool_call_counts).detach().item()
        total_tool_counts = tool_call_counts

    for key, value in batch.non_tensor_batch.items():
        if not key.startswith("tool_call_counts/"):
            continue
        tool_counts = torch.as_tensor(value).float()
        metrics[f"{key}/mean"] = torch.mean(tool_counts).detach().item()
        if total_tool_counts is not None:
            total_sum = torch.sum(total_tool_counts).detach().item()
            if total_sum > 0:
                ratio = torch.sum(tool_counts).detach().item() / total_sum
            else:
                ratio = 0.0
            metrics[f"{key}/ratio"] = ratio
